...
E=8.0108831e-14; %en J, Accelerer du repos avec 50KeV
lambda=sqrt((6.62607*10^-34)^2/(2*m*E));
% lamda=5.4847*10^-12;    
kp=2*(pi/lambda)*[1,0];
% kp=1e17*[1,0];
vitesse=kp(1)*hbar/m;
...
v_mat=sparse( ( barr_simple(x,y,6*10^(-16),x(length(x)/2),10^(-12),y(length(y)/2),0,'Gauss') )' );
V=v_mat(:);
...
norm_transmise=trapeze_2D(abs(Psy_mat(:,76:1:150,k)).^2,x(length(x)/2),x(end),y(1),y(end),Imax/2-1,Jmax-1);
norm_reflechie=trapeze_2D(abs(Psy_mat(:,1:75,k)).^2,x(1),x(length(x)/2),y(1),y(end),Imax/2-1,Jmax-1);
