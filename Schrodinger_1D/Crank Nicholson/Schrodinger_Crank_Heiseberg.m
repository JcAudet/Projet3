...
vid = VideoWriter('Heisenberg','MPEG-4');
vid.FrameRate = 150;
vid.Quality = 100;
open(vid)
...
for j=1:length(t)-1
    ...
    tic
    D1 = C * transpose(Psy1(j,:));
    Psy1(j+1,2:end-1)= mldivide(A,D1);
    D2 = C * transpose(Psy2(j,:));
    Psy2(j+1,2:end-1)= mldivide(A,D2);
    D3 = C * transpose(Psy3(j,:));
    Psy3(j+1,2:end-1)= mldivide(A,D3);
